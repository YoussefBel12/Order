...
const OrderManagement = () => {
        ...
        const fetchOrders = async () => {
            try {
                ...
                setOrders(data.map(order => ({
                    ...order,
                    createdDate: new Date(order.createdDate || order.orderDate)
                })));
            } catch (err) {
    ...
    const addOrder = async () => {
        try {
            ...
            setOrders(prev => [...prev, {
                ...data,
                createdDate: new Date(data.createdDate || data.orderDate)
            }]);
                        ...
                        <TableBody>
                            {orders.map((order) => (
                                <TableRow key={order.id} hover>
                                    ...
                                    <TableCell>
                                        <Chip
                                            label={`$${order.orderAmount.toFixed(2)}`}
                                            color="primary"
                                        />
                                    </TableCell>
                                    <TableCell>
                                        <Tooltip title={order.createdDate.toLocaleString()}>
