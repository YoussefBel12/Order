...
*/
//up is code before adding image suppot on products
...
const StockManagement = () => {
    ...
    const [productImage, setProductImage] = useState(null); // New state for image
    const [addWarehouseForm, setAddWarehouseForm] = useState({
        name: '',
        location: '',
        isActive: true
    ...
    const handleAddProduct = async () => {
        try {
            ...
            formData.append('isActive', addProductForm.isActive);
            if (productImage) {
                formData.append('image', productImage);
            }

            await axios.post(`${API_BASE_URL}/Stock/product`, formData, {
                ...config,
                headers: {
                    ...config.headers,
            ...
            {loading ? (
                ...
                <TableContainer component={Paper} sx={{ borderRadius: 2 }}>
                    <Table>
                        ...
                        <TableBody>
                            {stocks.map((stock) => {
                                // Find the product for this stock to get the imageUrl
